...
class EnhancedStableReward:
    ...
    def get_reward(self, warped_features, fixed_features, flow_field, 
        ...
        with torch.no_grad():
            ...
            # 2. Smoothness Cost (L1 penalty / Total Variation for robustness)
            dx = torch.mean((flow_field[:, :, :, :, 1:] - flow_field[:, :, :, :, :-1]).abs())
            dy = torch.mean((flow_field[:, :, :, 1:, :] - flow_field[:, :, :, :-1, :]).abs())
            dz = torch.mean((flow_field[:, :, 1:, :, :] - flow_field[:, :, :-1, :, :]).abs())
