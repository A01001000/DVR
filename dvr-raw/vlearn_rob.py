...
def run_registration_episode(policy, target_policy, transformer, f_mr_3d, f_ct_3d,
    ...
    for step in range(current_max_steps):
        ...
        # new_flow_acc = incremental_flow_full_res
        flow_update = flow_acc + incremental_flow_full_res
        current_max_flow = policy.get_current_max_flow()
        new_flow_acc = torch.clamp(flow_update, -current_max_flow, current_max_flow)
        ...
        if len(replay_buffer) >= hyperparams['BATCH_SIZE'] and total_steps % hyperparams['UPDATE_EVERY_N_STEPS'] == 0:
            ...
            with torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                with torch.no_grad():
                    # This is stateless and adapts to the changing reward scale from the curriculum
                    # r is the tensor of raw rewards (costs) from the buffer
                    # r_mean = r.mean()
                    # r_std = r.std() + 1e-8  # Add epsilon for numerical stability
                    # r_normalized = (r - r_mean) / r_std
                    
                    # Apply a larger clip value *after* per-batch normalization
                    # r_normalized = torch.clamp(r_normalized, -20.0, 20.0) 
                    ...
                    reward_clip_range = 20.0 # A reasonable starting point
                    ...
                    value_target = r.view(-1, 1) + hyperparams["GAMMA"] * next_value_clipped * (1 - d.view(-1, 1))
                ...
                with torch.no_grad():
                    ...
                    advantage = r.view(-1, 1) - torch.min(current_value1, current_value2).detach()
                    # advantage = value_target.detach() - torch.min(current_value1, current_value2).detach()
                    ...
                    # Use a small clamp for the *normalized* advantage
                ...
                new_log_p_actor = new_dist_actor.log_prob(a).mean(dim=(1,2,3,4)).view(-1, 1)

                # b: log_prob of that action under the OLD policy
                old_log_p_actor = old_dist_actor.log_prob(a).mean(dim=(1,2,3,4)).view(-1, 1)
                ...
                # We minimize -(log_prob * advantage)
                pg_loss = -(new_log_p_actor * advantage.detach())

                # The final actor loss is the policy gradient loss + the entropy bonus
                # (We are minimizing the negative entropy, i.e., maximizing entropy)
                actor_loss = pg_loss.mean() - alpha.detach() * entropy
            ...
            del next_value1, next_value2, next_value, next_value_clipped, value_target
            ...
            del alpha, entropy, actor_loss, alpha_loss
    ...
    del n_step_buffer, replay_buffer, f_mr_3d, f_ct_3d, final_v_next, state_tensor, done, G
...
def train_vlearn(dino_encoder, pca_transformer, feature_loader, val_loader, device, epochs=100, max_steps=5, save_dir="checkpoints", dino_dir=None, is_continue_training=False, use_raw_images=False, dataset_type="abdomen"):
    ...
    for epoch in range(epochs):
        ...
        SIM_WEIGHT = 50.0  # Similarity cost is our anchor
        
        # Define the weights to start with at Epoch 0
        SMOOTH_WEIGHT_START = 1.0  # 10x stronger
        MAG_WEIGHT_START = 0.5   # 10x stronger
        
        # Define end weights
        SMOOTH_WEIGHT_END = 5.0  # 10x stronger
        MAG_WEIGHT_END = 2.0   # 10x stronger
        
        # Define how many epochs to ramp up the penalties
        PENALTY_ANNEALING_EPOCHS = 75 
        ...
        hyperparams["SIMILARITY_WEIGHT"] = SIM_WEIGHT
