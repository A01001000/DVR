...
class SPAC:
    def __init__(self, stn, device=None):
        ...
        self.planner_optim = torch.optim.Adam(self.planner.parameters(), lr=cfg.LEARNING_RATE, betas=(0.5, 0.9))
    ...
    def choose_action(self, state, test=False):
        ...
        # --- ✅ DEFINITIVE AND FINAL FIX ---
        # Get the base field from the actor network.
        field = self.actor(latent, enc)
        
        # Add a small amount of uniform noise to "kick-start" the process.
        # This ensures the initial deformations are not zero and will generate a reward.
        # The network will quickly learn to produce deformations larger than this noise.
        if not test:
            noise = (torch.rand_like(field) - 0.5) * 0.1 # Small noise in range [-0.05, 0.05]
            field = field + noise

        # We no longer need scaling or clamping here.
        # --- END OF FIX ---
    ...
    def _loss(self, samples, update_planner, update_actor):
        ...
        # This prevents the exploding gradients that were destabilizing the critic.
        self.critic1_optim.zero_grad()
        loss['critic1'].backward()
        torch.nn.utils.clip_grad_norm_(self.critic1.parameters(), 1.0) # Clip gradients to a max norm of 1.0
        self.critic1_optim.step()

        self.critic2_optim.zero_grad()
        loss['critic2'].backward()
        torch.nn.utils.clip_grad_norm_(self.critic2.parameters(), 1.0) # Clip gradients to a max norm of 1.0
        ...
        if update_actor:
            ...
            # --- NEW MULTIMODAL LOSS ---
            similarity_loss = utils.local_mutual_information_loss(s[:, :1], warped)
            smoothness_loss = utils.gradient_loss(flow)
            kl_regularization = self.kl_loss(vae_dist)
            
            reg_loss = (1.0 * similarity_loss) + (0.5 * smoothness_loss) + (0.01 * kl_regularization)
            loss['reg'] = reg_loss
    ...
    def load_model(self, name, model_path):
        ...
        if name == 'actor':
            ...
            self.critic1.load_state_dict(state_dict)
        elif name == 'critic2':
            self.critic2.load_state_dict(state_dict)
        else:
            raise ValueError(f"Unknown model name: {name}")
            
        print(f"Successfully loaded {name} ({sum(p.numel() for p in state_dict.values()):,} parameters)")
