...
def numpy(data, device=None):
    if isinstance(data, np.ndarray):
        ...
        # If it's already a numpy array or other type, try to convert
...
def mutual_information(img1, img2, bins=64, sigma=1.0, normalized=True):
    """
    Calculates the Mutual Information between two images.
    This version is numerically stable and uses .item() to avoid TypeErrors.
    """
    # --- FIX: Convert tensor min/max to a Python number using .item() ---
    min_val = min(img1.min().item(), img2.min().item())
    max_val = max(img1.max().item(), img2.max().item())
    # --- END FIX ---

    # Ensure there's a range to create bins over
    if max_val == min_val:
        return torch.tensor(0.0, device=img1.device)
        
    # Create bin centers for the histogram
    bin_centers = torch.linspace(min_val, max_val, bins, device=img1.device).unsqueeze(1)
    
    # Flatten the images and use a Gaussian kernel to create a differentiable joint histogram
    x_flat = img1.view(img1.shape[0], -1)
    y_flat = img2.view(img2.shape[0], -1)
    
    gauss_x = torch.exp(-((x_flat.unsqueeze(1) - bin_centers)**2) / (2 * sigma**2))
    gauss_y = torch.exp(-((y_flat.unsqueeze(1) - bin_centers)**2) / (2 * sigma**2))

    # Normalize the Gaussian responses to sum to 1 for each pixel
    gauss_x = gauss_x / (gauss_x.sum(1, keepdim=True) + 1e-6)
    gauss_y = gauss_y / (gauss_y.sum(1, keepdim=True) + 1e-6)

    # Compute the joint histogram by matrix multiplication
    p_xy = torch.bmm(gauss_x, gauss_y.transpose(1, 2))
    # Normalize the joint histogram to get the joint probability distribution
    p_xy = p_xy / (p_xy.sum() + 1e-6)

    # Calculate marginal probabilities
    p_x = p_xy.sum(dim=2)
    p_y = p_xy.sum(dim=1)

    # Calculate entropies, adding epsilon to prevent log(0)
    epsilon = 1e-7
    ent_x = -torch.sum(p_x * torch.log(p_x + epsilon), dim=1)
    ent_y = -torch.sum(p_y * torch.log(p_y + epsilon), dim=1)
    ent_xy = -torch.sum(p_xy * torch.log(p_xy + epsilon), dim=(1, 2))

    # Calculate Mutual Information
    mi = ent_x + ent_y - ent_xy
    
    if normalized:
        # Normalized Mutual Information (NMI) - handle division by zero
        nmi = (ent_x + ent_y) / (ent_xy + epsilon)
        return nmi
    else:
        return mi

def local_mutual_information_loss(I, J):
    """The loss function is the NEGATIVE of the mutual information."""
    return -mutual_information(I, J, normalized=True).mean()

def local_mutual_information_score(I, J):
    """The score for the environment's reward is the POSITIVE MI."""
    return mutual_information(I, J, normalized=True).mean()
...
class MINDLoss(nn.Module):
    """
    Modality Independent Neighborhood Descriptor (MIND) loss for 3D images.
    Reference: Heinrich et al., "MIND: Modality Independent Neighbourhood Descriptor for Multi-modal Deformable Registration", MedIA 2012
    """
    def __init__(self, patch_size=2, dilation=2):
        super().__init__()
        self.patch_size = patch_size
        self.dilation = dilation
        # Define 6-neighborhood offsets for 3D
    ...
    def forward(self, x, y):
        # x, y: [B, 1, D, H, W]
        mind_x = self._mind_descriptor(x)
        mind_y = self._mind_descriptor(y)
        # L1 loss between descriptors
        return torch.mean(torch.abs(mind_x - mind_y))

    def _mind_descriptor(self, img):
        # img: [B, 1, D, H, W]
        B, C, D, H, W = img.shape
        device = img.device
        pad = self.patch_size * self.dilation
        img_pad = F.pad(img, (pad, pad, pad, pad, pad, pad), mode='replicate')
        patches = []
        for shift in self.shifts:
            dz, dy, dx = shift
            shifted = img_pad[:, :,
                pad+dz*self.dilation:pad+D+dz*self.dilation,
                pad+dy*self.dilation:pad+H+dy*self.dilation,
                pad+dx*self.dilation:pad+W+dx*self.dilation]
            patches.append(shifted)
        patches = torch.stack(patches, dim=1)  # [B, 6, 1, D, H, W]
        diffs = (img.unsqueeze(1) - patches) ** 2  # [B, 6, 1, D, H, W]
        # Patch sum
        kernel = torch.ones((1, 1, self.patch_size, self.patch_size, self.patch_size), device=device)
        ssd = F.conv3d(diffs.view(B*6, 1, D, H, W), kernel, padding=0)
        ssd = ssd.view(B, 6, 1, D - self.patch_size + 1, H - self.patch_size + 1, W - self.patch_size + 1)
        # Normalize
        mind = torch.exp(-ssd / (ssd.mean(dim=1, keepdim=True) + 1e-8))
        return mind.squeeze(2)  # [B, 6, D', H', W']
