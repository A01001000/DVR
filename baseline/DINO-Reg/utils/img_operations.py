...
def apply_deformation_and_compute_dice(fixed_seg_arr, moving_seg_arr, disp, num_classes, case_id, output_dir):
    """
    ...
    warped_seg = warp_arr(moving_seg_arr, disp)
    
    # --- DEBUG: Save the warped segmentation mask ---
    # This is the most important image to look at!
    debug_dir = os.path.join(output_dir, 'debug')
    os.makedirs(debug_dir, exist_ok=True)
    nib.save(nib.Nifti1Image(warped_seg, np.eye(4)), os.path.join(debug_dir, f'debug_case{case_id}_warped_seg.nii.gz'))
    # --- END DEBUG ---
    
    dice = []

    labels_to_check = num_classes if isinstance(num_classes, list) else range(1, num_classes + 1)

    for i in labels_to_check:
        fixed_mask = (fixed_seg_arr == i)
        warped_mask = (warped_seg == i)
        
        if not np.any(fixed_mask) and not np.any(warped_mask):
            dice.append(np.nan)
        else:
            dice.append(compute_dice_coefficient(fixed_mask, warped_mask))
