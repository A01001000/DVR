...
def train_model(t1_dir, dwi_dir, model_label, epochs, img_size, lr, batch_size, cont_training, data_name, device):
    """
    ...
    save_dir = f'{model_label}_1_mind_{weights[0]}_diffusion_{weights[1]}_{data_name}/'  # Directory to save logs and experiments
    ...
    if model_label == "MIDIR":
        ...
        model = VoxelMorph(
            inshape=img_size,
            nb_unet_features=None,  # Use default features
            nb_unet_levels=None,    # Use default levels
            unet_feat_mult=1        # Default multiplier
        )  # Initialize VoxelMorph model with proper parameters
    else:
    ...
    # Choose appropriate loss function based on model and data type
    if data_name.lower() in ['mrct', 'mr_ct', 'multimodal']:
        # For multimodal MR-CT registration, use NCC instead of MIND
        print("Using Normalized Cross Correlation loss for multimodal MR-CT registration")
        # Alternative options: NCCLoss(), MutualInformationLoss()
        criterion = NCCLoss()  # Normalized Cross Correlation for multimodal registration
        criterions = [criterion, Grad3d(penalty='l2')]  # Add gradient regularization
        weights = [1, 0.01]  # Adjust weights for multimodal registration
    else:
        # For monomodal registration, use MIND
        criterion = MINDLoss()  # MIND loss function
        criterions = [criterion, Grad3d(penalty='l2')]  # Add gradient loss
        weights = [1, 1]  # Original weights
    ...
    for epoch in range(epoch_start, epochs):
        ...
        # Clear unused memory between iterations
