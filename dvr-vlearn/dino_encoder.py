...
class DINOEncoder(nn.Module):
    def __init__(self, device='cuda', patch_size=14, dino_version='dinov2'):
        super().__init__()
        self._device = device
        ...
        if self.dino_version == 'dinov2':
            self.patch_size = 14
            self.resize = Resize((224, 224), antialias=True)  # 224 / 14 = 16
            print(f"DINOv2 (ViT-L/14) initialized. Input resize: 224x224 -> 16x16 grid.")
        elif self.dino_version == 'dinov3':
            self.patch_size = 16
            self.resize = Resize((256, 256), antialias=True)  # 256 / 16 = 16
            print(f"DINOv3 (ViT-L/16) initialized. Input resize: 256x256 -> 16x16 grid.")
        else:
            raise ValueError(f"Unknown DINO version: {dino_version}")
        
        self.dino = self.load_dino(self.dino_version)
        self.dino.eval()
    ...
    def load_dino(self, dino_version):
        """
        Loads the specified DINO model (DINOv2 or DINOv3).
        """
        if dino_version == 'dinov2':
            print("Loading DINOv2 model (vit_large) from local path...")
            # Import DINOv2 locally
            from dvr2.dinov2.vision_transformer import vit_large 
            ...
            if not os.path.exists(model_path):
                raise FileNotFoundError(
                    f"DINOv2 model checkpoint not found at '{model_path}'. "
        ...
        elif dino_version == 'dinov3':
            print("Loading DINOv3 model (dinov3_vitl16) from torch.hub...")
            try:
                # We use vitl16 to be comparable to DINOv2's vitl14
                dino = torch.hub.load('facebookresearch/dinov3', 'dinov3_vitl16', pretrained=True)
                print("DINOv3 model loaded successfully.")
                return dino.to(self._device)
            except Exception as e:
                print(f"Failed to load DINOv3 from torch.hub: {e}")
                print("Please ensure you have an internet connection and the 'dinov3' repo is accessible.")
                raise e
        
        else:
            raise ValueError(f"Unknown DINO version specified: {dino_version}")
    ...
    def extract_dino_features(self, slice_batched_tensor):
        ...
        with torch.no_grad():
            if self.dino_version == 'dinov2':
                # DINOv2 uses the forward_features method
                features_dict = self.dino.forward_features(img3c)
            else: # 'dinov3'
                # DINOv3 (from torch.hub) uses the forward method with flags
                features_dict = self.dino(img3c, return_patches=True, return_class_token=False)
            
            # Both models should return this key based on their respective vision_transformer.py
