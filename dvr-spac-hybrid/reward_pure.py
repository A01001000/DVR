...
class RawImageMINDSystem:
    """
    Enhanced reward with exploration bonus to prevent conservative collapse.
    """
    def __init__(self, device):
        self.device = device
        self.baseline_cache = {}  # Cache baseline costs
    ...
    def mind_descriptor(self, img, radius=1, dilation=1):
        ...
        dist = torch.mean((img.unsqueeze(1) - out) ** 2, dim=2)
        dist_var = torch.mean((dist - torch.mean(dist, dim=1, keepdim=True))**2, 
                             dim=1, keepdim=True)
    ...
    def get_reward(self, warped_features, fixed_features, flow_field, 
        ...
        FIXED reward with:
        1. Exploration bonus
        2. Normalized improvement scaling
        3. Minimum movement requirement
        """
        with torch.no_grad():
            # Compute MIND descriptors
            ...
            # === FIX 1: Compute improvement with proper scaling ===
            if baseline_features is not None:
                mind_baseline = self.mind_descriptor(baseline_features)
                baseline_cost = ((mind_baseline - mind_fixed)**2).mean()
                improvement = baseline_cost - similarity_cost
                
                # === KEY INSIGHT: Normalize by baseline ===
                # If baseline cost is 0.2, improving to 0.15 is 25% improvement
                relative_improvement = improvement / (baseline_cost + 1e-6)
                
                # Scale to reasonable range: 25% improvement → +50 reward
                similarity_reward = relative_improvement * 200.0
                
            else:
                # Penalize absolute cost
                similarity_reward = -similarity_cost * 100.0
            
            # === FIX 2: Add exploration bonus (decreases over time) ===
            flow_magnitude = torch.norm(flow_field, p=2, dim=1).mean()
            
            # Early training: reward movement
            if epoch < 30:
                exploration_bonus = torch.clamp(flow_magnitude, 0, 5.0) * 2.0
            elif epoch < 60:
                exploration_bonus = torch.clamp(flow_magnitude, 0, 3.0) * 1.0
            else:
                exploration_bonus = 0.0
            
            # === FIX 3: Penalize doing nothing ===
            if flow_magnitude < 0.5:
                movement_penalty = -10.0  # Strong penalty for no movement
            else:
            ...
            # Adaptive penalties
            if flow_magnitude > 15.0:
                penalty_scale = 3.0
            elif flow_magnitude > 10.0:
                penalty_scale = 2.0
            else:
                penalty_scale = 1.0
            ...
            total_reward = torch.clamp(total_reward, -50.0, 50.0)
            
            # Enhanced logging
            if torch.rand(1).item() < 0.02:
                print(f"\n  [Enhanced Reward Debug]")
                print(f"    Baseline cost: {baseline_cost.item():.4f}")
                print(f"    Current cost: {similarity_cost.item():.4f}")
                print(f"    Improvement: {improvement.item():.4f} ({relative_improvement.item()*100:.1f}%)")
                print(f"    Similarity reward: {similarity_reward.item():.1f}")
                print(f"    Exploration bonus: {exploration_bonus.item():.1f}")
                print(f"    Movement penalty: {movement_penalty:.1f}")
                print(f"    Total reward: {total_reward.item():.1f}")
                print(f"    Flow magnitude: {flow_magnitude.item():.2f}")
