...
from dvr2.models_rob import Planner, Actor, VCritic, SpatialTransformer, VLearnPolicy, VLearnCritic
...
def evaluate_vlearn(test_loader, save_dir, vol_shape, model_dir, device,
                    diner_encoder, pca_transformer, use_raw_images, max_steps=5, dataset_type=None):
    """
    UPDATED: Evaluates the VLearn policy, saves metrics to CSV, and generates detailed PNGs.
    *** NOW WORKS WITH VLearnPolicy INSTEAD OF Planner/Actor ***
    """
    ...
    # --- Load the new VLearnPolicy model ---
    ...
    def load_model_state(model, key, checkpoint):
        if key in checkpoint:
            ...
            raise RuntimeError(f"ERROR: Could not find key '{key}' in checkpoint. Checkpoint keys: {checkpoint.keys()}")

    try:
        # --- Load the policy model ---
        load_model_state(policy, 'policy_state_dict', checkpoint)
    ...
    for batch_idx, (mr_vol_batch, ct_vol_batch, batch_spacing, mr_seg_batch, ct_seg_batch) in enumerate(tqdm(test_loader, desc="Evaluating")):
        ...
        with torch.no_grad():
            ...
            if use_raw_images:
                f_mr_batch = mr_vol_batch.unsqueeze(1)
                ...
                for i in range(batch_size):
                    ...
                    f_mr, f_ct = extract_single_dino_features(diner_encoder, mr_vol_3d, ct_vol_3d, pca_transformer, dataset_type=dataset_type)
            ...
            for step in range(max_steps):
                ...
                # 2. Get action from VLearnPolicy
                action_dist = policy(current_state)
                incremental_flow_low_res = action_dist.mean # Use deterministic mean for eval
...
def validate_memory_efficient(policy, critic, val_loader, device, vol_shape, feature_shape, 
                              max_steps=5, max_validation_samples=None, 
                              use_raw_images=False, dino_encoder=None, pca_transformer=None, dataset_type=None):    
    """
    CORRECTED: Memory-efficient validation for VLearnPolicy.
    *** NOW WORKS WITH VLearnPolicy INSTEAD OF Planner/Actor ***
    """
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(tqdm(val_loader, desc="Validating")):
            ...
            for sample_idx in range(batch_size):
                ...
                f_mr_low = F.interpolate(f_mr, size=FEATURE_SHAPE_LOW, mode='trilinear', align_corners=False)
                f_ct_low = F.interpolate(f_ct, size=FEATURE_SHAPE_LOW, mode='trilinear', align_corners=False)
                ...
                for step in range(max_steps):
                    ...
                    # --- GET ACTION FROM VLearnPolicy ---
                    action_dist = policy(current_state)
                    incremental_flow_low_res = action_dist.mean # Use mean for validation
                ...
                del f_mr, f_ct, mr_vol, ct_vol, flow_acc, warped_mr
