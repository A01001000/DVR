...
## Baseline Models
Each baseline model (SyN, VoxelMorph, NestedMorph, Dino-Reg, SPAC) contains its own folder in the 'baseline' directory with imported code from their original repositories. The final results of these baseline models are in the 'final_results' folder. Firstly, apply data augmentation by running aug.slurm. Then, run the respective files in the 'slurm' folder to train and test them for each of the datasets:\
