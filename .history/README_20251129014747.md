...
## Baseline Models
Each baseline model (SyN, VoxelMorph, NestedMorph, Dino-Reg, SPAC) contains its own folder in the 'baseline' directory with imported code from their original repositories. The final results recorded are in the 'final_results' folder. Run the respective files in the 'slurm' folder to train and test them for each of the datasets:
* To get initial metrics for datasets: test-initial.slurm
* SyN: syn.slurm
* VoxelMorph: vm-train.slurm & vm-test.slurm
* NestedMorph: nm-train.slurm & vm-test.slurm
