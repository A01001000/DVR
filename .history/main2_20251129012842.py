...
if __name__ == '__main__':
    ...
    dino_dir = os.path.join(args.model_dir, "Dino-Features", f"{model_name}")
    model_dir = os.path.join(args.model_dir, "Models", f"{model_name}")
    ...
    if args.train:
        ...
        if args.dino:
            ...
            train_dino_head(dino_encoder, train_paths, dino_dir, target_size=(128, 128, 128), head_dataset=head_dataset, set_type="train")
        ...
        val_data_cached = precompute_features_in_memory(val_image_loader, dino_encoder, pca_transformer, device)

        # The DataLoader will automatically call __getitem__ and apply the augmentation
        # for each item in a batch, using multiple worker threads.
        ...
        print(f"Created a training dataset with {len(train_dataset)} original image pairs.")
        print("On-the-fly augmentation will be applied to each sample during training.")
        ...
        train_vlearn(dino_encoder, pca_transformer, feature_loader, val_loader, device, epochs=args.epochs, max_steps=args.steps, save_dir=model_dir, dino_dir=dino_dir, is_continue_training=args.resume)
...
""" 
    if args.test:
        ...
        metrics = evaluate_vlearn(test_loader, test_folder, vol_shape, feature_shape, model_dir, device=device)
